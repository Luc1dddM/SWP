...
namespace SWP_CarService_Final.Services
{
    public class TeamMemberService : DBContext
    {
        ...
        public User GetLeaderExist(string teamId)
        {
            User user = null;
            try
            {
                ...
                SqlCommand command = new SqlCommand("select [User].[UserName], " +
                                                    ...
                                                    "join [User_role] on [User_role].[userName] = [User].[UserName] " +
                                                    "join [Role] on [Role].[role_id] = [User_role].[role_id] " +
                                                    "right join [Team_Members] on [Team_Members].[userName] = [User].[UserName] " +
                                                    "right join [Team] on [Team].[team_id] = [Team_Members].[team_id] " +
                                                    "where [Role].[role_name] = 'leader' and [Team_Members].[team_id] = @team_id", connection);
                command.Parameters.AddWithValue("team_id", teamId);
                using(SqlDataReader reader = command.ExecuteReader())
                {
                    if(reader.Read())
